...
import { StatusBar, Easing, TouchableWithoutFeedback, Dimensions, Animated, Text, View, StyleSheet } from 'react-native';
...
export default class App extends Component {
  constructor(props) {
    ...
    closeItems.forEach(i => {
      this.closeAnimations.push(new Animated.Value(i === 0 ? -CLOSE_MODE : CLOSE_MODE))
    })
  ...
  renderBurger() {
    return <View style={[styles.closeContainer, styles.burgerContainer, {position: 'absolute', top: 0, right: 0}]}>
        <Animated.View style={[styles.line, styles.lineMedium, {
          transform: [{
        ...
        <Animated.View style={[styles.line, styles.lineSmall, {
          transform: [{
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <Animated.View style={[StyleSheet.absoluteFill, {backgroundColor: bgColor}]}></Animated.View>
        <Animated.View style={[styles.menuContainer, StyleSheet.absoluteFill, {backgroundColor: 'transparent', opacity: opacity, transform: [{translateY: translateContent}]}]}>
          <View style={{flex: 1, alignItems: 'center', justifyContent: 'space-around', backgroundColor: 'transparent'}}>
            <TouchableWithoutFeedback onPress={this._closeDrawer}><Text style={styles.buttonStyle}>Login</Text></TouchableWithoutFeedback>
            <TouchableWithoutFeedback onPress={this._closeDrawer}><Text style={styles.buttonStyle}>Create account</Text></TouchableWithoutFeedback>
            <TouchableWithoutFeedback onPress={this._closeDrawer}><Text style={styles.buttonStyle}>Support</Text></TouchableWithoutFeedback>
            <TouchableWithoutFeedback onPress={this._closeDrawer}><Text style={styles.buttonStyle}>About</Text></TouchableWithoutFeedback>
          </View>
        </Animated.View>
        <View style={{backgroundColor: 'transparent', position: 'absolute', transform: [{
          rotate: '-35deg',
        }],
        }}>
          <Animated.View style={[styles.strip, styles.top, {height: this.state.animateStripe, transform: [{
            translateY: top
          }] }]}></Animated.View>
          <Animated.View style={[styles.strip, styles.bottom, {height: this.state.animateStripe, transform: [{
            translateY: bottom
          }]}
          ]}></Animated.View>
        </View>
        <Animated.Image  source={{uri: 'https://ui8.s3.amazonaws.com/v5/assets/global/touch-icon-ipad-retina.png'}} 
        style={[StyleSheet.absoluteFill, styles.image, {opacity: this.state.animateOpacity, transform: [{
          scale: scaleLogo
        }]}]}/>
      ...
      <TouchableWithoutFeedback onPress={() => {
        ...
        <View style={[styles.closeContainer, styles.burgerContainer, { 
          transform: [{
            rotate: '-45deg'
          }]
        }]}>
